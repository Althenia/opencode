...
const Endpoint3_9 = (raw: RawClient["server.session"]) => (input: Endpoint3_9Input) =>
  raw["session.goal.start"]({ params: { sessionID: input["sessionID"] }, payload: { goal: input["goal"] } }).pipe(
    Effect.mapError(mapClientError),
    Effect.map((value) => value.data),
  )

type Endpoint3_10Request = Parameters<RawClient["server.session"]["session.goal.stop"]>[0]
...
type Endpoint3_11Request = Parameters<RawClient["server.session"]["session.goal.status"]>[0]
...
const adaptGroup3 = (raw: RawClient["server.session"]) => ({
  ...
  goalStop: Endpoint3_10(raw),
  goalStatus: Endpoint3_11(raw),
