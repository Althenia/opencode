...
const smallModel = ModelV2.Info.make({
  ...
  name: "Small model",
  api: { type: "aisdk", package: "@ai-sdk/openai", id: ModelV2.ID.make("small-api"), url: "https://test.invalid" },
...
const lineageDependencies = (admit: SelfImprovementAdmission.Interface["admit"]) =>
  Layer.mergeAll(
    ...
    Layer.mock(PluginV2.Service, { wait: () => Effect.void }),
...
test("waits for variants before reading generation models or their provider", async () => {
  await Effect.runPromise(
    program(
      Effect.gen(function* () {
        ...
        expect(calls).toEqual(["wait", "default", "small", "provider"])
      }),
    ),
  )
})
    ...
    program(
      Effect.gen(function* () {
        ...
        const dependencies = Layer.mergeAll(
          ...
          Layer.mock(LLMClient.Service, {
            generate: (request) => Effect.sync(() => requests.push(request)).pipe(Effect.andThen(Effect.fail(failure))),
          }),
          Layer.mock(SelfImprovementAdmission.Service, {
