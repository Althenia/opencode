...
```bash
opencode run --model openai/gpt-5.2 "Explain this repository"
opencode run -m openai/gpt-5.2#high "Review the current changes"
```
...
- The root `model` currently sets the default provider and model only. Although its selection shape accepts a variant,
  the V2 catalog default does not retain it; select a variant in the TUI, with `opencode run`, or on an agent or command.
