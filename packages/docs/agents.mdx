...
OpenCode preserves inherited permission ceilings on the child, and re-evaluates
the parent's `subagent` permission before every model-originated control.

## Durable subagent tasks

Each managed subagent is a child Session with a durable task record. The parent
receives a bounded TeamView in model context with each direct child's task,
agent, selected model and variant, state, latest progress, and open question.
Child history and full results remain in the child Session.

The `subagent` tool accepts `agent`, `description`, `prompt`, and `background`.
It also accepts an optional canonical model override:
...
OpenCode resolves the model before creating the child. Precedence is the
spawn-time override, the subagent's configured model, the parent Session model,
then the Location catalog default. An unavailable model, unsupported provider
package, or unknown variant fails without creating a child task or pending
input.

A foreground child normally blocks until it returns final assistant text. A
background child returns immediately and notifies the parent after terminal
settlement. If a foreground child asks its parent a question, OpenCode durably
moves the task to `waiting`, detaches the local job, and returns control to the
parent. Answering admits one durable child input and resumes execution.

Parents use `subagent_control` to:

- list direct child tasks;
- send `steer` input at the next safe step boundary;
- send `queue` input when the child would otherwise become idle;
- answer the child's one open question;
- cancel a child; or
- resume a nonterminal child only when durable pending work already exists.

Children use `subagent_report` to record bounded progress or ask one durable
parent question. Progress updates TeamView but does not wake the parent.
Questions, detached completion or failure, cancellation, and lost execution
notify and wake the parent through a transactional outbox.

Task communication survives process restart. OpenCode never automatically
replays an in-flight provider attempt; that task becomes `lost`. A launch whose
input was admitted before any physical attempt may receive an advisory recovery
wake.

Only the invoking parent identity controls its direct managed children. A
sibling, unrelated Session, or fork Session cannot control the task. Siblings
exchange relevant information only through their parent. These ownership checks
are structural API controls and are not tenant isolation.
...
The TUI uses this as the preferred model when the agent is selected. Child model
selection follows the durable subagent precedence described above. In the API,
the session's selected model is stored separately; creating or switching a
primary session with only an agent ID does not itself change that session model.
