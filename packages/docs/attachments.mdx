...
Pass `--file` or `-f` to `opencode run`. Repeat the flag for multiple files:

```bash
opencode run -f src/server.ts -f screenshot.png "Explain the failure"
```
...
```bash
opencode api post /api/session/ses_example/prompt --data '{
